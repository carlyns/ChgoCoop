...
<hr id="cooplist">
	<h2>Chicagoland Housing Co-ops (<a href="https://www.google.com/maps/d/u/0/viewer?mid=1XihqVgdsrpQRCvE-2q5KsEsj0iY&ll=41.86774747699223%2C-87.69356691594237&z=12" target="_blank">See Our Map</a>)</h2>
	<p>
		(Last updated manually updated by volunteers Jan 2017)
	</p>
	<p>Don't see your co-op? <a href="<?php get_page_template(); ?>/chgocoopWP/enhance#cooplisting">Get Listed!</a></p>
	...
	<div class="cooplist-item">
		<div class="coopname">
			<h3><?php the_title(); ?>, <?php the_field('bldgsize'); ?></h3>
		</div>
		...
		<?php // if statement for 'checkbox' acf === basic listing
			...
			<p>(No Profile, <a href="http://maps.google.com/?q=<?php the_field('intersection'); ?>" target="intersection"><?php the_field('address'); ?>google map</a>)</p>


			<?php // if statement for 'checkbox' acf === full profile
		 	elseif ( $listing && in_array('full',$listing) ): ?>
			<a  class="profiletag" href="<?php the_permalink(); ?>"><div><p class="black-link-font">Profile available</p></div></a>
      ...
      if( $vacancy ): ?>
        <?php foreach( $vacancy as $vacancy): ?>
					<div class="vacancytag">
							<a href="<?php echo get_permalink($vacancy->ID); ?>"><p class="black-link-font">unit vacancy: (<?php the_field('bedrooms', $vacancy->ID); ?>)</p></a>
					</div>
